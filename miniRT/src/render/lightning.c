...
/*   Updated: 2023/11/06 19:31:22 by abeltran         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector normal)
{
	t_vector	l_dir;
	double		intensity;
	t_vector	sphere_color;
	t_object	*lights;
	t_object	*aux;

	lights = data->lights;
	intensity = 0;
	ft_vector_init(&sphere_color, obj->color->i, obj->color->j, obj->color->k);
	while (lights)
	{
	...
	ft_apply_final_lights(data, &sphere_color, intensity);
	return (sphere_color);
}
